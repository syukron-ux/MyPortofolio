	...
	class Gutentor_Widget_Blog_Post_Templates {
		...
		public function template_1( $data, $post, $attributes ) {
			...
			if ( $attributes['blockSingleItemButtonEnable'] ) {
				...
				$output       .= '<a class="' . esc_attr( gutentor_concat_space( $default_class, GutentorButtonOptionsClasses( $icon_options ) ) ) . '" ' . esc_attr( apply_filters( 'gutentor_save_link_attr', '', esc_url( get_permalink() ), $link_options ) ) . '>' . $icon . '<span>' . esc_html( $attributes['buttonText'] ) . '</span></a>';
			}
