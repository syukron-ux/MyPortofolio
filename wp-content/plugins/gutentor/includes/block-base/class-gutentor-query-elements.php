	...
	class Gutentor_Query_Elements {
		...
		public function get_button( $post, $attributes ) {
			...
			if ( isset( $attributes['pOnBtn'] ) && $attributes['pOnBtn'] ) {
				...
				$output      .= '<a class="' . esc_attr( gutentor_concat_space( $default_class, $custom_class, $icon_position_class ) ) . '" ' . esc_attr( apply_filters( 'gutentor_save_link_attr', '', esc_url( get_permalink() ), $link_options ) ) . '>' . $icon . '<span>' . esc_html( $attributes['pBtnText'] ) . '</span></a>';
			}
		...
		public function get_preview_button( $post, $attributes ) {
			...
			if ( isset( $attributes['q1OnBtn'] ) && $attributes['q1OnBtn'] ) {
				...
				$output      .= '<a class="' . esc_attr( gutentor_concat_space( $default_class, $custom_class, $icon_position_class ) ) . '" ' . esc_attr( apply_filters( 'gutentor_save_link_attr', '', esc_url( $demo_url ), $link_options ) ) . '>' . $icon . '<span>' . esc_html( $attributes['q1BtnTxt'] ) . '</span></a>';
			}
			return $output;
		}
		...
		public function get_featured_post_button( $post, $attributes ) {
			...
			if ( isset( $attributes['pOnFPBtn'] ) && $attributes['pOnFPBtn'] ) {
				...
				$output .= '<a class="' . esc_attr( gutentor_concat_space( $default_class, $icon_position_class ) ) . '" ' . esc_attr( apply_filters( 'gutentor_save_link_attr', '', esc_url( get_permalink() ), $link_options ) ) . '>' . $icon . '<span>' . esc_html( $attributes['pFPBtnText'] ) . '</span></a>';
			}
			return $output;
		}
		...
		public function get_term_button( $term, $attributes ) {
			...
			if ( isset( $attributes['tOnBtn'] ) && $attributes['tOnBtn'] ) {
				...
				$output      .= '<a class="' . esc_attr( gutentor_concat_space( $default_class, $custom_class, GutentorButtonOptionsClasses( $icon_options ) ) ) . '" ' . esc_attr( apply_filters( 'gutentor_save_link_attr', '', esc_url( get_term_link( $term ) ), $link_options ) ) . '>' . $icon . '<span>' . esc_html( $attributes['tBtnTxt'] ) . '</span></a>';
			}
			return $output;
		}
